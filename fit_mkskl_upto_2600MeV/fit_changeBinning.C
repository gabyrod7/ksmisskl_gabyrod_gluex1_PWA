...
void fit_changeBinning() {
	...
	TFile *opf = TFile::Open("rootFiles/fit_changeBinning.root", "RECREATE");
	...
	for(string hName : hNames) {
		pars = fit_2bw_no_interference(hName, 1.16, 2.60, hName);
		...
		fit_2bw_with_interference(hName, 1.16, 2.60, hName);
