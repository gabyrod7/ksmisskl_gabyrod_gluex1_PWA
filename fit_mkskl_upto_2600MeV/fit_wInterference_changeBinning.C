...
void fit_wInterference_changeBinning() {
	gStyle->SetOptStat(0);
	...
	TFile *opf = TFile::Open("rootFiles/fit_wInterference_changeBinning.root", "RECREATE");

	vector<double> pars;
	vector<string> hNames = {"im_kskl_binning1", "im_kskl_binning2"};

	int count = 0;
	for(string hName : hNames) {
		pars = fit_2bw_with_interference(hName, 1.16, 2.60, hName);

		m1->SetPoint(count, count, pars[0]);
		m1->SetPointError(count, 0, pars[1]);
		g1->SetPoint(count, count, pars[2]);
		g1->SetPointError(count, 0, pars[3]);
		m2->SetPoint(count, count, pars[4]);
		m2->SetPointError(count, 0, pars[5]);
		g2->SetPoint(count, count, pars[6]);
		g2->SetPointError(count, 0, pars[7]);
		phase->SetPoint(count, count, pars[8]);
		phase->SetPointError(count, 0, pars[9]);
