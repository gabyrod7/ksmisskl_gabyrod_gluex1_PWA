...
vector<double> fit_2bw_no_interference(string hName, double min = 1.16, double max = 2.60);
vector<double> fit_2bw_with_interference(string hName, double min = 1.16, double max = 2.60);
...
void fit_changeEventSelections() {
	...
	vector<string> hNames = {"im_kskl", "im_kskl_mpipi1", "im_kskl_mpipi2", "im_kskl_mpipi3", "im_kskl_mmiss1", "im_kskl_mmiss2", "im_kskl_mandel_tp1", "im_kskl_mandel_tp2",
		...
		"im_kskl_beam_energy1", "im_kskl_beam_energy2"};
	...
	TGraphErrors *m1_ni = new TGraphErrors(),
		*g1_ni = new TGraphErrors(),
		*m2_ni = new TGraphErrors(),
		*g2_ni = new TGraphErrors(),

		*m1_wi = new TGraphErrors(),
		*g1_wi = new TGraphErrors(),
		*m2_wi = new TGraphErrors(),
		*g2_wi = new TGraphErrors(),
		*p_wi = new TGraphErrors();
	...
	for(string hName : hNames) {
		...
		pars = fit_2bw_no_interference(hName);
		...
		fit_2bw_with_interference(hName);

		count++;
		frame->GetXaxis()->SetBinLabel(count, hNamesMap[hName].c_str());
		// frame->LabelOptions("v", "X");
		// U_against_SN_before->LabelsOption("v", "X");
...
vector<double> fit_2bw_with_interference(string hName, double min = 1.16, double max = 2.60) {
	...
	fit->SetParLimits(6, 0, 2*TMath::Pi());
	fit->SetLineWidth(3);
	fit->SetNpx(1000);
