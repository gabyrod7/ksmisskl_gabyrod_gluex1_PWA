...
void fit_wInterference_pol1() {
	...
	TF1 *bkg = new TF1("bkg", mybkg, min, max, 2);
	...
	sig->Draw("same");
	bkg->SetParameters(fit->GetParameter(7), fit->GetParameter(8));
	bkg->Draw("SAME");
	...
	TLatex t;
	t.DrawLatex(1.25, 0.93*h->GetMaximum(), "Bkg: pol1");

	c->SaveAs("pdf_wInterference/bkgPol1.pdf");
	...
	TFile *outf = new TFile("rootFiles/fit_wInterference_pol1.root", "RECREATE");
	m1->Write("m1");
	m2->Write("m2");
	g1->Write("g1");
	g2->Write("g2");
