...
void fit_2bw_noInterference_pol1() {
	gStyle->SetOptStat(0);
	...
	sig->Draw("same");
	bkg->SetParameters(fit->GetParameter(6), fit->GetParameter(7));
	bkg->Draw("SAME");
	...
	c->SaveAs("pdf_nInterference/bkgPol1.pdf");
	...
	TFile *outf = new TFile("rootFiles/fit_pol1.root", "RECREATE");
	m1->Write("m1");
	m2->Write("m2");
	g1->Write("g1");
	g2->Write("g2");
